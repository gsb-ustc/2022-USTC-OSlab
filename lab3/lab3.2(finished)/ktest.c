...
// 定义要输出的文件
#define OUTPUT_FILE "/home/gsb/oslab/lab3.2/lab3.2/expr_result.txt"
...
static my_page_referenced mpage_referenced = (my_page_referenced)0xffffffff96a75d20;
// sudo cat /proc/kallsyms | grep follow_page
static my_follow_page mfollow_page = (my_follow_page)0xffffffff96a5c9c0;
...
static void print_mm_active_info(void)
{
    ...
    if (mm) {
        ...
        while (vma) {
            ...
            int freq;
            for (vm_addr; vm_addr < vma->vm_end; vm_addr += (1 << 12)) {
                struct page* page = mfollow_page(vma, vm_addr, FOLL_GET);
                if (!IS_ERR_OR_NULL(page)) {
                    unsigned long vm_flags;
                    freq = mpage_referenced(page, 0, page->mem_cgroup, &vm_flags);
...
static unsigned long virt2phys(struct mm_struct *mm, unsigned long virt)
{
    ...
    pgd_t* pud_addr = pgd_offset(mm, virt);
    pud_t* pmd_addr = pud_offset((p4d_t*)pud_addr, virt);
    pmd_t* pte_addr = pmd_offset(pmd_addr, virt);
    pte_t* phy_addr = pte_offset_kernel(pte_addr, virt);
    page = pte_page(*phy_addr);
    if (page)
    {
...
static void traverse_page_table(struct task_struct *task)
{
    ...
    if (mm)
    {
        ...
        while (vma) {
            ...
            int freq;
            for (vm_addr; vm_addr < vma->vm_end; vm_addr += (1 << 12)) {
                record_two_data(vm_addr, virt2phys(task->mm, vm_addr));
            }
            vma = vma->vm_next;
        }
        flush_buf(1);
    ...
    if(ktest_func == 4)
{
    ...
    while (vma) {
        ...
        for (vm_addr; vm_addr < vma->vm_end; vm_addr += (1 << 12)) {
            ...
            if (!IS_ERR_OR_NULL(page)) {
                char* addr;
                if (vm_addr >= start_data_addr && vm_addr + (1 << 12) <= end_data_addr)
                {
                ...
                else if (vm_addr <= start_data_addr && vm_addr + (1 << 12) >= start_data_addr && vm_addr + (1 << 12) <= end_data_addr)
                {
                    addr = kmap_atomic(page);
                    memcpy(buf, addr + start_data_addr - vm_addr, vm_addr + (1 << 12) - start_data_addr);
                    curr_buf_length += vm_addr + (1 << 12) - start_data_addr;
                    kunmap_atomic(addr);
                    
                    flush_buf(0);
                }
                else if (vm_addr >= start_data_addr && vm_addr <= end_data_addr && vm_addr + (1 << 12) >= end_data_addr)
                {
                    addr = kmap_atomic(page);
                ...
                else if (vm_addr <= start_data_addr && vm_addr + (1 << 12) >= end_data_addr)
                {
                    addr = kmap_atomic(page);
